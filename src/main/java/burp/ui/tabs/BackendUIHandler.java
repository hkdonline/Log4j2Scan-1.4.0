...
public class BackendUIHandler {
    public enum Backends {
        BurpCollaborator, DnslogCN, Ceye, RevSuitDNS, RevSuitRMI, GoDnslog,SelfDigPm, DigPm, Eyes, Antenna
    }
    ...
    private JTextField antennaApiKeyInput;
    private JTextField antennaDomainInput;
    private JTextField antennaAPIInput;
    ...
    public JPanel getPanel() {
        ...
        backendsPanel.addTab("Antenna", getAntennaPanel());
    ...
    private JPanel getAntennaPanel() {
        JPanel panel1 = new JPanel();
        panel1.setAlignmentX(0.0f);
        panel1.setBorder(new EmptyBorder(10, 10, 10, 10));
        panel1.setLayout(new BoxLayout(panel1, 1));
        ...
        JPanel subPanel1 = UIUtil.GetXJPanel();
        antennaDomainInput = new JTextField(200);
        antennaDomainInput.setMaximumSize(antennaDomainInput.getPreferredSize());
        subPanel1.add(new JLabel("doMain: "));
        subPanel1.add(antennaDomainInput);


        JPanel subPanel2 = UIUtil.GetXJPanel();
        antennaApiKeyInput = new JTextField(200);
        antennaApiKeyInput.setMaximumSize(antennaApiKeyInput.getPreferredSize());
        subPanel2.add(new JLabel("API Key: "));
        subPanel2.add(antennaApiKeyInput);

        JPanel subPanel3 = UIUtil.GetXJPanel();
        antennaAPIInput = new JTextField(200);
        antennaAPIInput.setMaximumSize(antennaAPIInput.getPreferredSize());
        subPanel3.add(new JLabel("API Address: "));
        subPanel3.add(antennaAPIInput);

        JPanel subPanel4 = UIUtil.GetXJPanel();
        JButton saveBtn = new JButton("Save");
        saveBtn.setMaximumSize(saveBtn.getPreferredSize());
        saveBtn.addActionListener(e -> {
            Config.set(Config.ANTENNA_TOKEN, antennaApiKeyInput.getText());
            Config.set(Config.ANTENNA_ROOT_DOMAIN, antennaDomainInput.getText());
            Config.set(Config.ANTENNA_REST_API, antennaAPIInput.getText());
            JOptionPane.showMessageDialog(mainPanel, "Save success!");
        });
        JButton applyBtn = new JButton("Save&Apply");
        applyBtn.setMaximumSize(applyBtn.getPreferredSize());
        applyBtn.addActionListener(e -> {
            saveBtn.doClick();
            Config.set(Config.CURRENT_BACKEND, Backends.Antenna.name());
            this.loadConfig();
            this.apply();
        });
        saveBtn.setMargin(buttonMargin);
    ...
    private void loadConfig() {
        ...
        //Antenna Input
        antennaApiKeyInput.setText(Config.get(Config.ANTENNA_TOKEN));
        antennaDomainInput.setText(Config.get(Config.ANTENNA_ROOT_DOMAIN));
        antennaAPIInput.setText(Config.get(Config.ANTENNA_REST_API));
    }
