...
public class Utils {
    ...
    public static String formatArray(String[] arr) {
        StringBuffer sb = new StringBuffer("%5b");
        for (String i : arr) {
            sb.append(String.format("%%22%s%%22%%2C", i));
        }
        sb.replace(sb.length() - 1, sb.length(), "%5d");
        return sb.toString();
    }
    ...
    public static String getNewPayload(IBackend backend, IRequestInfo req){
        String host = Utils.formatHex(req.getHeaders().get(1).replace("Host: ", ""));
        String tmpDomain = "";
        String url = Utils.formatHex(req.getUrl().toString());
        if(backend.getName().equals("Antenna")){
            if (url.length() <= 200){
                StringBuilder sb = new StringBuilder();
                for(int i =0; i<url.length(); i+= 60){
                    if ((i+60) < url.length()){
                        sb.append(url, i, i+60);
                        sb.append('.');
                    }else {
                        sb.append(url, i, url.length());
                    }
                }
                host = sb.toString();
            }
            tmpDomain = backend.getNewPayload(host);
        }else {
            tmpDomain = backend.getNewPayload();
        }
        return tmpDomain;
    }
